...
def draw_labeled_box(image, x, y, w, h, label, color):
    ...
    cv2.putText(
        image, f"Card {label}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2
    )
...
def combine_images(images, cols=2):
    ...
    for i, img in enumerate(images):
        r, c = divmod(i, cols)
        combined[r * height : (r + 1) * height, c * width : (c + 1) * width] = (
...
def get_corner_snip(flattened_images: list):
    ...
    for img in flattened_images:
        ...
        bin_img = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[
            1
        ]
...
def match_card(rank_image, suit_image, ranks_path, suits_path, threshold=0.8):
    ...
    for rank_file in os.listdir(ranks_path):
        ...
        result = cv2.matchTemplate(rank_image, template_image, cv2.TM_CCOEFF_NORMED)
    ...
    for suit_file in os.listdir(suits_path):
        ...
        result = cv2.matchTemplate(suit_image, template_image, cv2.TM_CCOEFF_NORMED)
        _, max_val, _, max_loc = cv2.minMaxLoc(result)
...
while True:
    ...
    for idx, contour in enumerate(contours):
        ...
        min_contour_area = (
            100  # Anda dapat menyesuaikan threshold ini sesuai dengan kebutuhan
        )
